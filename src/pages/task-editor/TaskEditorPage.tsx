...
const TaskEditorPage: FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg shadow-sm m-x-4 m-b-4 overflow-hidden border border-[#eef0f2]">
      ...
      <Modal title="任务设置" open={settingsOpen} onCancel={() => setSettingsOpen(false)}
        footer={<Button onClick={() => setSettingsOpen(false)}>关闭</Button>} width={520}>
        {editor.currentTask && (
          <div className="flex flex-col gap-6 pt-2">
            <section>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-4 rounded-full bg-[#3b82f6]" />
                <label className="text-[13px] font-bold text-[#1a1a2e]">基本信息</label>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {(["名称", "版本", "起始步骤", "作者"] as const).map((label, i) => (
                  <div key={label}>
                    <span className="text-[11px] font-medium text-[#6b7280] mb-1 block">{label}</span>
                    {i === 2 ? (
                      <Select className="w-full" size="small" placeholder="选择第一个执行的步骤" allowClear
                        value={editor.currentTask!.start || undefined}
                        options={stepKeys.map((k) => ({ value: k, label: k }))}
                        onChange={(v) => editor.updateStart(v ?? "")} />
                    ) : (
                      <Input value={[editor.currentTask!.name, editor.currentTask!.version, "", editor.currentTask!.author][i]} readOnly size="small" className="bg-[#f9fafb]" />
                    )}
                  </div>
                ))}
              </div>
              <div className="mt-3">
                <span className="text-[11px] font-medium text-[#6b7280] mb-1 block">描述</span>
            ...
            <section>
              <div className="flex items-center gap-2 mb-3">
                ...
                <label className="text-[13px] font-bold text-[#1a1a2e]">监控配置</label>
              </div>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-medium text-[#6b7280]">间隔(秒)</span>
                  <InputNumber value={editor.currentTask.monitors.interval ?? 1} min={0.1} step={0.1} size="small" style={{ width: 90 }}
                    onChange={(v) => editor.updateMonitors({ ...editor.currentTask!.monitors, interval: v ?? 1 })} />
                </div>
                <div className="flex items-center gap-2 flex-1">
                  <span className="text-[11px] font-medium text-[#6b7280]">循环步骤</span>
                  <Select mode="tags" className="flex-1" size="small" placeholder="输入步骤名"
                    value={editor.currentTask.monitors.loop ?? []}
                    options={stepKeys.map((k) => ({ value: k, label: k }))}
                    onChange={(v) => editor.updateMonitors({ ...editor.currentTask!.monitors, loop: v })} />
                </div>
              </div>
            </section>
          </div>
